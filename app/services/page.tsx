  ...
  return (
    <>
      ...
      <section className="py-20 md:py-28 bg-slate-50">
        <div className="max-w-content mx-auto px-6 md:px-10">
          <ScrollReveal variant="fadeUp" className="text-center max-w-2xl mx-auto mb-14">
            ...
            <p className="text-slate-600 text-lg">Four core services that cover the full product lifecycle, from idea to launch and beyond.</p>
          </ScrollReveal>
          <div className="grid sm:grid-cols-2 gap-8">
            {services.map((s, i) => (
              <ScrollReveal key={s.href} variant="scale" delay={i * 80}>
              <div
                className="group bg-white rounded-2xl border border-slate-200 p-8 shadow-sm hover:shadow-xl hover:border-cyan/30 transition-all duration-300"
              >
                <span className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-cyan/10 text-cyan mb-6 group-hover:bg-cyan/20 transition-colors">
                  {s.icon}
                </span>
                <span className="text-xs font-headline font-bold text-cyan/80 mb-2 block">0{i + 1}</span>
                <h3 className="font-headline font-bold text-xl text-navy mb-3">{s.title}</h3>
                <p className="text-slate-600 text-sm leading-relaxed mb-5">{s.description}</p>
                <ul className="flex flex-wrap gap-2 mb-6">
                  {s.features.map((f) => (
                    <li key={f} className="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-medium">{f}</li>
                  ))}
                </ul>
                <div className="flex flex-wrap items-center gap-3">
                  <Link
                    href="/contact"
                    className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-cyan hover:bg-cyan-dark text-navy font-body font-semibold rounded-lg text-sm transition-colors"
                  >
                    Get a quote
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                  </Link>
                  <Link href={s.href} className="inline-flex items-center gap-1.5 text-cyan font-body font-medium text-sm hover:text-cyan-dark">
                    Learn more
                  </Link>
                </div>
              </div>
      ...
      <section className="py-20 md:py-28 bg-slate-50">
        <div className="max-w-content mx-auto px-6 md:px-10">
          <ScrollReveal variant="fadeUp" className="text-center max-w-2xl mx-auto mb-14">
            <h2 className="font-headline font-bold text-3xl md:text-4xl text-navy mb-4">How we work</h2>
            <p className="text-slate-600 text-lg">From first conversation to launch and beyond, clear phases, clear communication.</p>
          </ScrollReveal>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {processSteps.map(({ step, title, desc }, i) => (
              <ScrollReveal key={step} variant="fadeUp" delay={i * 80}>
              <div className="relative">
                <span className="font-headline font-bold text-4xl text-cyan/20 mb-4 block">{step}</span>
      ...
      <section className="py-16 md:py-20 bg-slate-50 border-y border-slate-200">
        <div className="max-w-content mx-auto px-6 md:px-10">
          <ScrollReveal variant="fadeUp" className="text-center mb-8">
            <h2 className="font-headline font-bold text-xl md:text-2xl text-navy">Technologies we use</h2>
          </ScrollReveal>
          <div className="flex flex-wrap justify-center gap-3">
            {techStack.map((t, i) => (
              <ScrollReveal key={t} variant="scale" delay={i * 40}>
              <span className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 font-medium text-sm shadow-sm">
                {t}
              </span>
              </ScrollReveal>
            ))}
